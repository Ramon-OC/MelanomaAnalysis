function smooth_image = color_butterworth_lowpass(original_image, order, cut_frequency)
	    
    [m, n] = size(original_image(:, :, 1)); 

    for channel = 1:3
    % Fourier Transform of the original_image 
    fourier_transform = fft2(double(original_image(:, :, channel))); 
    ...
    original_image(:, :, channel) = uint8(ifft2(double(g)));
    end
    
    smooth_image = original_image;
